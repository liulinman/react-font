...
1. Run `node .agents/skills/impeccable/scripts/context.mjs` once per session (if the runtime shows this skill's loaded base directory, run `node <skill-base-dir>/scripts/context.mjs`; keep cwd at the user's project). Pass a named source file or route as `--target <path>`. This project-local copy disables update and staleness network/cache checks by default; only the user may opt in with `IMPECCABLE_ALLOW_UPDATE_CHECK=1` or `IMPECCABLE_ALLOW_STALENESS_CHECK=1`. It loads PRODUCT.md, DESIGN.md, the matching surface brief, and native-platform guidance when applicable; follow its directives and do not rerun it.
