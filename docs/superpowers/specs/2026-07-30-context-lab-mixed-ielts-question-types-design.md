...
- 移动端混合题型作答；保留现有移动端行为，本期不修改移动端文件。
...
移动端混合题型整体延期。本期不得修改 `englishWorldMobile` 下的页面、组件、
样式和测试；后续启用移动端 Story 时再实现标题、软键盘、固定提交栏和触控尺寸
要求。
...
- 提交并验证正确、错误、未作答和限词超限。
- 刷新后查看 Attempt 历史。
- 在 `1280x720`、`1440x900` 和 `1920x1080`
  检查无重叠、遮挡和整页横向滚动。
...
- 前端：题目/答案类型、Web 桌面渲染、草稿、结果、历史和 API 类型。
